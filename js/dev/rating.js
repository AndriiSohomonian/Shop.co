...
function initSliders() {
  ...
  const thumbsSliderEl = document.querySelector(".thumbs-slider");
...
function updateVisibleSlides(swiperInstance) {
  swiperInstance.slides.forEach(
    (slide2) => slide2.classList.remove("visible-slide")
  );
  const screenWidth = window.innerWidth;
  const activeIndex = swiperInstance.activeIndex;
  let visibleIndices = [];
  if (screenWidth <= 1150.76) {
  ...
  visibleIndices.forEach((index) => {
    const slide2 = swiperInstance.slides[index];
    if (slide2) {
      slide2.classList.add("visible-slide");
    }
  });
}
